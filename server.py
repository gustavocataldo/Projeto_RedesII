...
def broadcast_registry_table(clients: Dict[str, Client]) -> str:
    ...
    for nickname, info in clients.items():
        tabela = tabela + \
            f'{nickname}             {info.port}             {info.address}\n'
    ...
    while True:
        try:
            ...
            if message == '/quit':
                ...
                client_msg = (
                    f'QUERY_RESULT|{_client.port}-{_client.address}-{nickname}' if _client else 'NICKNAME_NOT_FOUND')
...
def receive(clients: Dict[str, Client]):
    while True:
        ...
        clients.setdefault(nickname, Client(
            socket=client, port=client_port, address=client_address, nickname=nickname))

        send_encoded_message(client, 'CLIENT_CONNECTED\n')

        thread = threading.Thread(
            target=handle, args=(clients[nickname], clients))
        thread.start()
